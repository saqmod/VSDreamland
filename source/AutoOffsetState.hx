...
class AutoOffsetState extends MusicBeatState
{
	...
	override function create() {	
		...
		if(!easterEgg){
			//Init BG
			bg = new FlxSprite(0, 0).loadGraphic(Paths.image('fpsPlus/offsetBG', "dreamland"));
			...
			//Init Speakers
			speakers.frames = Paths.getSparrowAtlas('fpsPlus/speaker', "dreamland");
			...
			//Init BG
			bg = new FlxSprite(0, 0).loadGraphic(Paths.image('fpsPlus/offsetBG-pixel', "dreamland"));
			...
			//Init Speakers
			speakers = new FlxSprite(0, 0).loadGraphic(Paths.image('fpsPlus/speaker-pixel', "dreamland"), true, 102, 52);
		...
		FlxG.camera.fade(FlxColor.BLACK, 0.5, true, function(){
			...
			FlxG.sound.playMusic(Paths.music("fpsPlus/offsetSong" + (easterEgg?"-pixel":""), "dreamland"), 1, false);
